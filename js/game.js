    ...
    document.querySelectorAll('.theme-option').forEach(option => {
        option.onclick = function() {
            ...
            if (state.inventory.themes.includes(theme)) {
                applyTheme(theme); ServerAPI.sync();`n    };
    });
}
...
const HolidayEngine = {
    holidays: [
        { id: 'may_1', name: '╨Я╤А╨░╨╖╨┤╨╜╨╕╨║ ╨▓╨╡╤Б╨╜╤Л', date: '2026-05-01', reward: 1000, art_id: 101 },
        { id: 'may_9', name: '╨Ф╨╡╨╜╤М ╨Я╨╛╨▒╨╡╨┤╤Л', date: '2026-05-09', reward: 2000, art_id: 102 }
    ],
    check() {
        const now = new Date();
        ...
        this.holidays.forEach(h => {
            ...
            if (diffDays <= 7 && diffDays > 0) {
                ...
                hubBonus.innerHTML = 'тП░ ' + h.name + ' ' + (state.lang === 'ru' ? '╤З╨╡╤А╨╡╨╖' : 'in') + ' ' + diffDays + ' ' + (state.lang === 'ru' ? '╨┤╨╜.' : 'd.');
                hubBonus.onclick = null;
            } else if (diffDays === 0) {
                activeHoliday = h;
                hubBonus.classList.remove('hidden');
                hubBonus.style.background = 'linear-gradient(90deg, #8b5cf6, #d946ef)';
                hubBonus.innerHTML = '╤А╤ЯтАЭтДЦ ' + h.name + '! ' + (state.lang === 'ru' ? '╨Ш╨У╨а╨Р╨в╨м ╨б╨Ю╨Т╨л╨в╨Ш╨Х' : 'PLAY EVENT');
    ...
    startEvent(h) {
        ...
        if (completed.includes(h.id)) {
            alert(state.lang === 'ru' ? '╨б╨╛╨▒╤Л╤В╨╕╨╡ ╤Г╢╨╡ ╨┐╤А╨╛╢╨┤╨╡╨╜╨╛!' : 'Event already completed!');
            return;
        }
    ...
    completeEvent() {
        ...
        updateUI();
        alert('╤А╤ЯтАЭтДЦ ' + h.name + ' ' + (state.lang === 'ru' ? '╨Я╨а╨Ю╨йд╨Х╨Э╨Ю!' : 'COMPLETED!') + ' +' + h.reward + ' ╤А╤Я╨ДтДв');
